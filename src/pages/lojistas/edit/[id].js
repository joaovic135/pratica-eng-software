...
export default function Editar() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (session?.user?.usuario?.tipoUsuario === 'admin') {
      ...
      router.push('/auth/lojista'); // Redireciona outros usuários para a página principal de lojistas
    }
  };

  if (session) {
    if (session?.user?.usuario?.tipoUsuario === 'admin' || session.user.lojista.id  === Number(id)){
      return (
